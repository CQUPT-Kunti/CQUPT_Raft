...
- 当前首个落点是 `GarbageCollector` 的 task model、bounded queue 和可注入删除 handler。

## 修改规则

- 保持命名空间统一为 `storedemo`。
- 这里只做后台任务模型、状态机、队列、重试和统计，不直接实现 metadata-driven safety check、candidate generation、restart resume。
