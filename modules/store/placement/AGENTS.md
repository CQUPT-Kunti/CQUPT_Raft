...
- 当前实现 `ReplicaPolicySelector::SelectReplicas()` 和 `PlacementManager::SelectPlacement()`。
...
- `PlacementManager` 负责协调输入、补充决策说明并复用 `ReplicaPolicySelector`，不要在 manager 层复制一套筛选排序逻辑。
