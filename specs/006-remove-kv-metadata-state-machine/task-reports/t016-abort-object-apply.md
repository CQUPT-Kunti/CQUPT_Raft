# T016 MetadataStateMachine AbortObject apply 最小逻辑

## 1. 结论

- T016 已完成。
- 本次在 `MetadataStateMachine` 中实现了 `AbortObject` 的最小 apply 逻辑。
- 本次只处理中止 `PENDING` 对象。
- 未实现 `DeleteObject`、完整 `request_id` 幂等、真实 snapshot、MetadataService 接入或默认 wiring 切换。

## 2. 实际修改

- 修改 `modules/raft/state_machine/metadata_state_machine.cpp`
  - 在 `MetadataStateMachine::Apply(...)` 中增加 `AbortObject` 分支
  - 成功 abort 后：
    - 将对象状态置为 `DELETED`
    - 写入 `delete_time`
  ...
  - 调整 `HeadObject(...)`：
    - 对 `DELETED` 对象返回 `NotFound`
    - 保证 aborted 对象不再被当作有效对象暴露
- 修改 `tests/metadata_state_machine_test.cpp`
  - 新增 `AbortObject` 成功路径测试
  - 新增 bucket 不存在、bucket 已删除、object 不存在、`object_id` 不匹配、对象已 `COMMITTED`、对象已 abort 等错误路径测试
  - 将原“不支持命令”测试更新为仍未实现的 `DeleteObject`
- 修改 `modules/raft/state_machine/AGENTS.md`
  - 同步说明 `MetadataStateMachine` 当前已承载 `AbortObject` 的最小终止态 apply

## 3. Abort 语义固定

- 能识别 `MetadataCommandType::kAbortObject`
- 要求 bucket 存在且未删除
- 要求 object 已存在
- 要求 `object_id` 匹配
- 只允许中止 `PENDING` 对象
...
- 对象已 abort：
  - `state conflict: object already aborted`
- 当前仍未实现的 object 命令继续返回：
  - `unsupported metadata command type: delete_object`

## 5. 可见性结果

- aborted 对象不会继续作为有效对象暴露
- `HeadObject(...)` 对 aborted 对象返回 `NotFound`
- `ListObjects(...)` 当前仍为空占位返回，因此 aborted 对象不会被列出

## 6. last_applied 语义

- 成功 apply `AbortObject` 后：
...
- 本任务不扩展 term 输入，因此保持 T012-T015 的占位语义
...
- Linux configure
  - 结果：`PASS`
  - 日志：`tmp/test-logs/t016-configure.log`
- Linux build
  - 结果：`PASS`
  - 日志：`tmp/test-logs/t016-build.log`
- Linux CTest
  - 结果：`PASS`
  - 统计：`18/18` 通过
...
- 当前只实现 `AbortObject` 最小 apply，不实现：
  - `DeleteObject`
...
- `AbortObject` apply 最小逻辑已实现
- `PENDING ObjectRecord` 能被成功 abort
- aborted 对象不会继续作为有效对象暴露
- `object_index_ / tombstone / object state` 的 abort 语义已固定
- bucket 不存在、bucket 已删除、object 不存在、`object_id` 不匹配、对象非 `PENDING` 都有明确错误
- `last_applied_index / last_applied_term` 按当前语义更新
- `MetadataStateMachine` 不依赖 KV
- 未修改 `RaftNode` 默认 wiring
- 未删除 KV
- 未实现 `DeleteObject / snapshot / service`
- 相关 Linux 构建与个别测试已通过
