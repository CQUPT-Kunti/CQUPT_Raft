...
- [ ] T008 [P] Add additive ViewNode protobuf contract in `proto/view.proto` with RegisterNode, HeartbeatNode, DiscoverMetadata, DiscoverStorage, and GetClusterView messages
...
- [ ] T010 [P] Define cluster config data structures and validation interfaces in `modules/cluster/cluster_config.h`
- [ ] T011 Implement cluster config validation and deterministic generation in `modules/cluster/cluster_config.cpp`
- [ ] T012 [P] Define durable node identity types and load/store interfaces in `modules/cluster/node_identity.h`
...
- [ ] T015 [P] Define ViewNode registry and role/status types in `modules/view/view_registry.h`
...
- [ ] T020 [P] Define ViewNode client adapter in `modules/view/view_client.h`
- [ ] T021 Implement ViewNode client adapter in `modules/view/view_client.cpp`
- [ ] T022 [P] Add payload boundary audit test in `tests/integrated_object_storage_e2e_test.cpp` ensuring metadata commands/manifests never include raw file payload
