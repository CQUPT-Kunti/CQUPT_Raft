...
- [x] T062 [US4] 定义 `StorageNodeHeartbeat`、`StorageNodeRegistry` 和 liveness 状态；实际修改：`modules/store/node/storage_node_registry.h`、`modules/store/node/storage_node_registry.cpp`、`modules/store/node/module-notes.md`、`modules/store/node/AGENTS.md`、`modules/store/AGENTS.md`、`tests/storage_heartbeat_registry_test.cpp`、`specs/007-storage-node-data-plane/task-reports/common-risk-notes.md`；验收：字段包含 node_id、capacity、used、available、chunk_count、health、disk_pressure、io_error_count、load、last_seen、node_liveness；依赖：T061
- [x] T063 [US4] 实现 `StorageNodeService` heartbeat/report/register 入口；实际修改：`modules/store/node/storage_node_service.h`、`modules/store/node/storage_node_service.cpp`、`modules/store/node/module-notes.md`、`modules/store/node/AGENTS.md`、`tests/storage_node_service_test.cpp`、`specs/007-storage-node-data-plane/task-reports/common-risk-notes.md`；验收：同 sequence 幂等，stale heartbeat 不覆盖新 facts；依赖：T062
- [x] T064 [US4] 实现 `StorageNodeClient` heartbeat/report/register 调用；实际修改：`modules/store/node/storage_node_client.h`、`modules/store/node/storage_node_client.cpp`、`modules/store/node/module-notes.md`、`modules/store/node/AGENTS.md`、`tests/storage_node_client_test.cpp`、`specs/007-storage-node-data-plane/task-reports/common-risk-notes.md`；验收：gRPC timeout/cancelled/unavailable/invalid_argument/io_error 映射明确，duplicate register / stale heartbeat / same-sequence / partial report merge 经真实 service+registry 链路验证；依赖：T063
- [x] T065 [US4] 将 registry facts 接入 `PlacementManager` eligibility；实际修改：`modules/store/placement/placement_manager.h`、`modules/store/placement/placement_manager.cpp`、`modules/store/placement/module-notes.md`、`modules/store/placement/AGENTS.md`、`tests/store_placement_manager_test.cpp`、`specs/007-storage-node-data-plane/task-reports/common-risk-notes.md`；验收：Placement 现可直接消费生产 `StorageNodeRegistry` snapshot/facts，并保持静态 candidates 路径与 T033/T034/T060 既有语义不变；依赖：T060、T062
- [x] T066 [US4] 将 registry facts 接入 read replica selection；实际修改：`modules/store/placement/replica_policy.cpp`、`modules/store/placement/replica_policy.h`、`modules/store/placement/module-notes.md`、`tests/support/storage_read_test_utils.h`、`tests/storage_read_integration_test.cpp`、`tests/store_placement_policy_test.cpp`、`specs/007-storage-node-data-plane/task-reports/common-risk-notes.md`、`specs/007-storage-node-data-plane/task-reports/t066-read-replica-registry-facts.md`；验收：read selection 消费生产 registry snapshot，跳过 stale/unavailable/overloaded/corrupted 副本，降权高 disk pressure 副本，并保留无 registry facts 的 manifest fallback 兼容路径；依赖：T045、T062
- [x] T067 [US4] 运行 US4 验证命令；实际修改：`specs/007-storage-node-data-plane/task-reports/t067-us4-registry-placement-validation.md`、`specs/007-storage-node-data-plane/tasks.md`；验收：`storage_heartbeat_registry`、`storage_node_service`、`storage_node_client`、`store_placement_policy`、`store_placement_manager`、`storage_read_integration`、`storage_read_chunk_contract` 当前 Linux 环境验证 PASS；依赖：T059-T066
...
- [x] T071 [US5] 实现 stale staging cleanup 和 partial write detection；实际修改：`modules/store/chunk/local_disk_chunk_store.h`、`modules/store/chunk/local_disk_chunk_store.cpp`、`modules/store/chunk/module-notes.md`、`tests/storage_node_recovery_test.cpp`、`specs/007-storage-node-data-plane/task-reports/t071-stale-staging-cleanup.md`、`specs/007-storage-node-data-plane/task-reports/common-risk-notes.md`、`specs/007-storage-node-data-plane/tasks.md`；验收：`Initialize()` 先扫描 `chunks/staging/`，按 `staging_cleanup_grace_period_ms` 清理 stale/partial staging 与空 staging 目录；fresh staging 不误删，cleanup 不影响 `chunks/live/` final chunk，cleanup 失败返回明确错误，cleanup 后 live rebuild 仍只基于 final/live chunk 工作；依赖：T070
- [x] T072 [US5] 实现 corrupted chunk quarantine；实际修改：`modules/store/chunk/local_disk_chunk_store.h`、`modules/store/chunk/local_disk_chunk_store.cpp`、`modules/store/chunk/module-notes.md`、`tests/local_disk_chunk_store_test.cpp`、`tests/storage_node_recovery_test.cpp`、`tests/storage_read_integration_test.cpp`、`specs/007-storage-node-data-plane/task-reports/t072-corrupted-chunk-quarantine.md`、`specs/007-storage-node-data-plane/task-reports/common-risk-notes.md`、`specs/007-storage-node-data-plane/tasks.md`；验收：读取或校验发现 size/checksum mismatch 后将 final chunk 移入 `chunks/quarantine/` 并把 index 状态更新为 `kQuarantined`，重启后 `RebuildIndexFromDisk()` 能恢复 quarantine 事实且坏块不作为健康副本返回；当前 Linux 环境下 `local_disk_chunk_store`、`storage_node_recovery`、`storage_read_integration`、`storage_read_chunk_contract` 定向验证 PASS；依赖：T071
- [x] T073 [US5] 覆盖 crash / recovery matrix；实际修改：`tests/storage_node_recovery_test.cpp`、`specs/007-storage-node-data-plane/task-reports/t073-storage-node-crash-recovery-matrix.md`、`specs/007-storage-node-data-plane/task-reports/common-risk-notes.md`、`specs/007-storage-node-data-plane/tasks.md`；验收：test-only recovery matrix 固定 staging-only / visible live final / stale staging cleanup / quarantine 等 crash seam 的本地文件事实恢复 contract，重启后 staging 不进入 live index、visible final 可恢复、quarantined 不作为 healthy live，且当前 Linux 环境下 `local_disk_chunk_store`、`storage_node_recovery`、`storage_cross_platform_durability` 定向验证 PASS；依赖：T071
- [x] T074 [US5] 覆盖 crash after rename before parent directory sync 的平台 contract；实际修改：`tests/storage_cross_platform_durability_test.cpp`、`specs/007-storage-node-data-plane/task-reports/t074-rename-before-directory-sync-contract.md`、`specs/007-storage-node-data-plane/task-reports/common-risk-notes.md`、`specs/007-storage-node-data-plane/tasks.md`；验收：新增 cross-platform durability matrix row，Linux 当前环境下固定“rename 后文件可见不等于 crash 后 durable，必须显式 parent directory sync 才算完成 contract”，Windows 仍明确为 contract-only / deferred，不允许 required durability operation no-op success；依赖：T069、T073
- [x] T075 [US5] 覆盖 Windows long path、UTF-8 path、permission denied、disk full 错误分类；实际修改：`tests/storage_cross_platform_durability_test.cpp`、`tests/store_durable_file_test.cpp`、`specs/007-storage-node-data-plane/task-reports/t075-path-error-classification.md`、`specs/007-storage-node-data-plane/task-reports/common-risk-notes.md`、`specs/007-storage-node-data-plane/tasks.md`；验收：补充 Linux 当前环境下的 UTF-8 safe path、reserved/traversal/name-too-long、permission denied 和 LocalDiskChunkStore disk-full/path/permission 分类测试；Windows long path / UTF-8 / permission denied / disk full / reserved name / sharing violation 继续以 contract-only / deferred 表达，不允许路径错误或 unsupported 行为 silent success；依赖：T014、T015
- [x] T076 [US5] 覆盖 orphan chunk metadata-driven GC 边界；实际修改：`tests/storage_delete_gc_test.cpp`、`specs/007-storage-node-data-plane/task-reports/t076-orphan-metadata-driven-gc.md`、`specs/007-storage-node-data-plane/task-reports/common-risk-notes.md`、`specs/007-storage-node-data-plane/tasks.md`；验收：用真实 `GarbageCollector + metadata-driven safety checker + LocalDiskChunkStore::DeleteChunk` 固定 pending-timeout / failed-upload / deleted-object 的 orphan cleanup contract：本地 orphan 不能直接删，必须先经过 metadata live-manifest safety check；committed live manifest 仍引用时 delete handler 不得运行，不再引用时才允许删除，pending/failed-upload/abort/deleted candidate 均保留 `metadata_boundary`；依赖：T055、T070
- [x] T077 [US5] 运行 US5 低并发恢复验证；实际修改：`specs/007-storage-node-data-plane/task-reports/t077-us5-recovery-validation.md`、`specs/007-storage-node-data-plane/tasks.md`；验收：`CTEST_PARALLEL_LEVEL=1` 下定向运行 `storage_node_recovery` 与 `storage_cross_platform_durability`，当前 Linux 环境 PASS；不新增生产代码、不扩展到全量回归或 Windows 验证；依赖：T068-T076
...
- [x] T079 [P] [US6] 添加 RepairManager 测试；实际修改：`tests/storage_scrub_repair_test.cpp`、`specs/007-storage-node-data-plane/task-reports/t079-repair-manager-test.md`、`specs/007-storage-node-data-plane/task-reports/common-risk-notes.md`、`specs/007-storage-node-data-plane/tasks.md`；验收：在既有 `storage_scrub_repair` 测试中新增 test-only `TestOnlyRepairRunner + RepairReplicaFactsLedger`，固定 under-replicated repair contract：healthy source + healthy target 选择、corrupted/quarantined/stale/unavailable/unhealthy source 过滤、unhealthy/overloaded/high-disk/full-disk/insufficient/stale target 过滤、source checksum mismatch 失败、target durable success 后才更新 facts、durable 失败不更新 facts、already-exists 同 checksum idempotent success、all-source / all-target failure 和 repeated repair 幂等当前 Linux 环境下由 `storage_scrub_repair` 定向验证 PASS；不新增生产 RepairManager、RepairChunk proto/RPC 或 metadata/Raft 写入；依赖：T078
- [x] T080 [P] [US6] 添加 RebalanceManager 测试骨架；实际修改：`tests/storage_rebalance_test.cpp`、`tests/CMakeLists.txt`、`specs/007-storage-node-data-plane/task-reports/t080-rebalance-manager-test.md`、`specs/007-storage-node-data-plane/task-reports/common-risk-notes.md`、`specs/007-storage-node-data-plane/tasks.md`；验收：新增独立 `storage_rebalance` test-only `TestOnlyRebalanceRunner + TestOnlyManifestLedger + TestOnlyCleanupCandidateLedger + TestOnlySourceCleanupLedger`，用真实 `LocalDiskChunkStore + StorageNodeRegistry + PlacementManager` 固定 target durable -> manifest coordination -> source cleanup 三阶段 contract，以及 target durable 失败、manifest update 失败、cleanup retry、bad source/target 过滤、already-exists 幂等 durable 和 repeated rebalance 幂等边界；当前 Linux 环境下由 `storage_rebalance|storage_scrub_repair` 定向验证 PASS；不新增生产 RebalanceManager、proto、RPC 或 metadata/Raft 写入；依赖：T079

### Implementation for User Story 6

- [x] T081 [US6] 在 `proto/storage_node.proto` 中补齐 `ScrubChunk` 和 `RepairChunk` RPC 字段；实际修改：`proto/storage_node.proto`、`tests/storage_node_client_test.cpp`、`specs/007-storage-node-data-plane/task-reports/t081-scrub-repair-proto.md`、`specs/007-storage-node-data-plane/task-reports/common-risk-notes.md`、`specs/007-storage-node-data-plane/tasks.md`；验收：新增 `ScrubChunkRequest/Fact/Result/Response` 与 `RepairChunkRequest/Fact/Result/Response`，并在 `StorageNodeService` proto service 上补齐 `ScrubChunk`/`RepairChunk` RPC；`ScrubChunk` 固定 checksum/size/state/quarantine/corrupted 本地检查事实，`RepairChunk` 固定 source facts + target durable write 所需 bytes/facts，不携带 object committed/deleted 决策，不新增 metadata/Raft 字段；同步最小 fake `StubInterface` 占位后，当前 Linux 环境下 `storage_node_proto`、裸 `protoc`/`grpc_cpp_plugin` codegen、`storage_scrub_repair|storage_rebalance|storage_node_service|storage_node_client` 定向验证 PASS；不实现生产 service/client、ScrubManager、RepairManager、RebalanceManager；依赖：T061
- [X] T082 [US6] 实现 `StorageNodeService::ScrubChunk` 和 `StorageNodeClient::ScrubChunk`；允许修改：`modules/store/node/storage_node_service.cpp`、`modules/store/node/storage_node_client.cpp`；验收：full checksum validation，mismatch 标记 corrupted/quarantine；依赖：T072、T081
- [x] T083 [US6] 实现 `ScrubManager` bounded background queue；实际修改：`modules/store/maintenance/scrub_manager.h`、`modules/store/maintenance/scrub_manager.cpp`、`modules/store/maintenance/module-notes.md`、`CMakeLists.txt`、`tests/storage_scrub_repair_test.cpp`、`specs/007-storage-node-data-plane/task-reports/t083-scrub-manager.md`、`specs/007-storage-node-data-plane/task-reports/common-risk-notes.md`、`specs/007-storage-node-data-plane/tasks.md`；验收：新增生产 `ScrubManager`，复用 bounded executor 提供 submit/drain/stop/query 和有界后台 scrub queue，用真实 `ChunkStore::StatChunk + StorageNodeRegistry + ReplicaPolicySelector` 输出 corrupted/lost/under-replicated facts 与 repair candidate，队列满返回 overloaded、stop 后拒绝新任务、foreground IO 不经过 scrub queue、重复 scrub 结果稳定，且当前 Linux 环境下 `storage_scrub_repair|storage_node_service|storage_node_client` 定向验证 PASS；不实现 RepairManager、RepairChunk copy flow、RebalanceManager、metadata/Raft 写入或 object payload 持久化；依赖：T082
- [x] T084 [US6] 实现 `RepairManager` task model；实际修改：`modules/store/maintenance/repair_manager.h`、`modules/store/maintenance/repair_manager.cpp`、`modules/store/maintenance/module-notes.md`、`modules/store/maintenance/scrub_manager.h`、`CMakeLists.txt`、`tests/storage_scrub_repair_test.cpp`、`specs/007-storage-node-data-plane/task-reports/t084-repair-manager-task-model.md`、`specs/007-storage-node-data-plane/task-reports/common-risk-notes.md`、`specs/007-storage-node-data-plane/tasks.md`；验收：新增生产 `RepairManager` task model，支持从 scrub/repair candidate 规划 source_node、target_node、chunk_id、expected checksum/size，并固定 queued/running/completed/failed/cancelled/retry_pending、progress、attempts、last_error、submit/lookup/list/cancel/retry/complete/fail、稳定 task_id、duplicate already_exists、queue/task capacity overload 等 contract；当前 Linux 环境下 `storage_scrub_repair|repair_manager|scrub_manager` 定向验证 PASS；不实现 RepairChunk copy flow、under-replicated detection、metadata/Raft 写入或 object payload 持久化；依赖：T083、T065
- [x] T085 [US6] 实现 `RepairChunk` copy flow；实际修改：`modules/store/node/storage_node_service.h`、`modules/store/node/storage_node_service.cpp`、`modules/store/node/storage_node_client.h`、`modules/store/node/storage_node_client.cpp`、`modules/store/node/module-notes.md`、`modules/store/maintenance/repair_manager.h`、`modules/store/maintenance/repair_manager.cpp`、`modules/store/maintenance/module-notes.md`、`tests/storage_node_service_test.cpp`、`tests/storage_node_client_test.cpp`、`tests/storage_scrub_repair_test.cpp`、`specs/007-storage-node-data-plane/task-reports/t085-repair-chunk-copy-flow.md`、`specs/007-storage-node-data-plane/task-reports/common-risk-notes.md`、`specs/007-storage-node-data-plane/tasks.md`；验收：新增 `StorageNodeService::RepairChunk` / `StorageNodeClient::RepairChunk` 和 `RepairManager::RunTask()` 最小生产 copy flow，固定 source checksum/size verified、corrupted/quarantined/missing source 拒绝、target durable 成功后才 completed、already_exists 同 checksum 幂等成功、already_exists 不同 checksum/size 返回 conflict/checksum mismatch，且当前 Linux 环境下 `storage_scrub_repair|repair_manager|storage_node_service|storage_node_client|repair_chunk` 定向验证 PASS；不实现 under-replicated detection、metadata manifest coordination、read-side repair、RebalanceManager、metadata/Raft 写入或 object payload 持久化；依赖：T084
- [x] T086 [US6] 实现 under-replicated detection；实际修改：`modules/store/maintenance/scrub_manager.h`、`modules/store/maintenance/scrub_manager.cpp`、`modules/store/maintenance/repair_manager.h`、`modules/store/maintenance/repair_manager.cpp`、`modules/store/maintenance/module-notes.md`、`tests/storage_scrub_repair_test.cpp`、`specs/007-storage-node-data-plane/task-reports/t086-under-replicated-repair.md`、`specs/007-storage-node-data-plane/task-reports/common-risk-notes.md`、`specs/007-storage-node-data-plane/tasks.md`；验收：新增生产 under-replicated detection 语义，`ScrubManager` 固定 healthy replica count / required replica count / missing replica count，并过滤 corrupted/quarantined/missing/stale/unavailable/unhealthy replica；新增 `RepairManager::SubmitUnderReplicatedTask()`，可把 completed scrub task 中的 under-replicated fact 稳定转换为 repair task，记录 source_node、target_node、chunk_id、expected checksum/size，并继续复用 T085 `RunTask()` 完成 target durable copy；当前 Linux 环境下 `storage_scrub_repair|under_replicated|repair_manager|scrub_manager` 定向验证 PASS；不实现 RebalanceManager、metadata manifest coordination、read-side repair、metadata/Raft 写入或 object payload 持久化；依赖：T085
- [x] T087 [US6] 实现 `RebalanceManager` task model 但不接入自动后台调度；实际修改：`modules/store/maintenance/rebalance_manager.h`、`modules/store/maintenance/rebalance_manager.cpp`、`modules/store/maintenance/module-notes.md`、`CMakeLists.txt`、`tests/storage_rebalance_test.cpp`、`specs/007-storage-node-data-plane/task-reports/t087-rebalance-manager-task-model.md`、`specs/007-storage-node-data-plane/task-reports/common-risk-notes.md`、`specs/007-storage-node-data-plane/tasks.md`；验收：新增生产 `RebalanceManager` task model，支持 capacity/hotspot/new node join/draining/maintenance task 的 submit/lookup/list/cancel/fail/complete/retry、稳定 task_id、source/target registry snapshot 校验、queue/task capacity overload 和“默认不自动 copy”边界；当前 Linux 环境下 `storage_rebalance|rebalance_manager|storage_scrub_repair` 定向验证 PASS；不实现 rebalance copy/verify、metadata manifest coordination、source cleanup、自动后台调度、task persistence、metadata/Raft 写入或 object payload 持久化；依赖：T086
- [x] T088 [US6] 实现 rebalance copy/verify/manifest coordination 占位流程；实际修改：`modules/store/maintenance/rebalance_manager.h`、`modules/store/maintenance/rebalance_manager.cpp`、`modules/store/maintenance/module-notes.md`、`tests/storage_rebalance_test.cpp`、`specs/007-storage-node-data-plane/task-reports/t088-rebalance-copy-verify-flow.md`、`specs/007-storage-node-data-plane/task-reports/common-risk-notes.md`、`specs/007-storage-node-data-plane/tasks.md`；验收：新增生产 `RebalanceManager::RunTask()`，按 source read -> target durable -> target verify -> manifest coordination callback -> source cleanup callback 的固定顺序推进 rebalance task，并在 manifest 失败时生成 cleanup/orphan candidate、cleanup 失败时进入 retryable cleanup 状态、repeated rebalance 跳过已成功阶段；当前 Linux 环境下 `storage_rebalance|rebalance_manager|storage_scrub_repair` 定向验证 PASS；不实现真实 metadata manifest update / Raft 提交、自动后台调度、task persistence、read-side repair、metadata/Raft 写入或 object payload 持久化；依赖：T087
- [x] T089 [US6] 运行 US6 低并发验证；实际修改：`specs/007-storage-node-data-plane/task-reports/t089-us6-scrub-repair-rebalance-validation.md`、`specs/007-storage-node-data-plane/tasks.md`；验收：在当前 Linux 环境下执行 `cmake --build --preset debug-ninja-low-parallel` 与 `CTEST_PARALLEL_LEVEL=1 ctest --test-dir build/linux -R "storage_scrub_repair|storage_rebalance" --output-on-failure`，命中 `storage_scrub_repair`、`storage_rebalance` 两个 US6 相关测试并全部 PASS；未修改生产代码、测试代码、proto 或 common-risk-notes；依赖：T078-T088
...
- [x] T092 运行 recovery/snapshot/catch-up 低并发回归；实际修改：`specs/007-storage-node-data-plane/task-reports/t092-recovery-snapshot-catchup-validation.md`、`specs/007-storage-node-data-plane/tasks.md`；验收：在当前 Linux 环境下先执行 `ctest --test-dir build/linux -N -R "recovery|snapshot|catch|restart"` 记录通配入口，再执行 `cmake --build --preset debug-ninja-low-parallel`；由于仓库当前 `test.sh` 的真实低并发恢复入口为 `./test.sh --group recovery`，且其内部固定 `CTEST_PARALLEL_LEVEL=1` 与 `^(RaftSnapshotRestartTest|RaftSnapshotRecoveryTest|RaftSnapshotCatchupTest)\\.` 精确匹配，因此实际使用 `CTEST_PARALLEL_LEVEL=1 ./test.sh --group recovery --skip-configure --skip-build` 运行 15 个 snapshot/recovery/catch-up 测试并全部 PASS；未修改生产代码、测试代码、proto 或 `common-risk-notes.md`；日志位于 `tmp/007/t092-*.log`；依赖：T077
- [x] T093 运行全量平台中立回归；允许修改：无生产文件，仅运行 `CTEST_PARALLEL_LEVEL=1 ./test.sh --group all`；验收：PASS 或按日志规则输出失败摘要；依赖：T090-T092
- [x] T094 [P] 更新 future validation 说明；实际修改：`specs/007-storage-node-data-plane/quickstart.md`、`specs/007-storage-node-data-plane/task-reports/t094-final-follow-up.md`、`specs/007-storage-node-data-plane/tasks.md`；验收：`quickstart.md` 已从 plan-phase 占位说明更新为“真实已实现验证入口 + T093 最新通过结论 + Windows 待验证边界”的 follow-up 文档，只记录真实已跑过或当前应使用的验证命令；当前确认 T093 已通过、Linux 侧可以收口，但不宣称 Windows PASS；依赖：T089
- [x] T095 [P] 检查 `ObjectRecord.chunks` / `ChunkRef` 兼容性未被破坏；实际修改：`tests/metadata_manifest_test.cpp`、`tests/storage_upload_integration_test.cpp`、`specs/007-storage-node-data-plane/task-reports/t095-manifest-chunkref-compatibility.md`、`specs/007-storage-node-data-plane/tasks.md`；验收：补充 metadata/common proto descriptor 断言，固定 `ChunkRef` / `ObjectRecord` / `CommitObjectRequest` 不暴露 `payload` 或任何 `bytes` 字段，而 `storage::WriteChunkRequest` / `storage::ReadChunkResponse` 继续承载 data-plane payload；补充 `MetadataStateMachine` snapshot round-trip 与 upload integration payload-marker 断言，证明 committed manifest / `HeadObject` / `ListObjects` / snapshot 仅保存 chunk refs 与 manifest facts，真实 payload 仍只留在 `LocalDiskChunkStore`；当前 Linux 环境下实际使用 `ctest --test-dir build/linux -R "MetadataManifestTest|storage_upload|storage_read" --output-on-failure` 运行 6 个相关测试并全部 PASS（原始 `metadata_manifest` 过滤器未命中 gtest-discover 用例名，因此按真实测试名调整）；未修改生产代码、proto 或 `common-risk-notes.md`；依赖：T039、T048
- [x] T096 生成最终实现报告摘要；实际修改：`specs/007-storage-node-data-plane/implementation-notes.md`、`specs/007-storage-node-data-plane/tasks.md`；验收：新增最终实现摘要文档，按阶段汇总 007 StorageNode data-plane 的实现范围、核心边界、验证结果、T093 特别说明、跨平台 contract 偏差、剩余风险与后续建议；明确 Linux 侧已可收口，Windows 仍待最终验证；未修改生产代码、测试代码、proto 或 `common-risk-notes.md`；依赖：T093
