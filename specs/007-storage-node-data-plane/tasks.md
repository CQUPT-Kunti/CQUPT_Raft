...
- [ ] T013 实现 Linux durable file 路径：`fsync`/`fdatasync`、same-filesystem rename、parent directory sync 和 POSIX error mapping；允许修改：`modules/store/storage_node/durable_file.cpp`、`tests/storage_node_durable_file_test.cpp`；验收：Linux 上 disk full/permission denied/IO error 分类明确，flush/publish/directory sync 失败不返回成功；依赖：T011、T012
- [ ] T014 实现 Windows durable file 路径：`FlushFileBuffers`、Windows file handle、`MoveFileEx`/`ReplaceFile` 风险边界、long path/UTF-8 path 错误分类；允许修改：`modules/store/storage_node/durable_file.cpp`、`tests/storage_node_durable_file_test.cpp`；验收：Windows 分支无 no-op success，无法等价 directory sync 时返回 explicit unsupported 或记录较弱 contract；依赖：T011、T012
- [ ] T015 实现路径 normalization 和 chunk 目录布局 helper；允许修改：`modules/store/storage_node/durable_file.h`、`modules/store/storage_node/durable_file.cpp`、`tests/storage_node_durable_file_test.cpp`；验收：拒绝绝对路径逃逸、`..`、非法 chunk id、Windows reserved names；依赖：T011
- [ ] T016 定义 `ChunkIndex` 接口和 sharded map 结构；允许修改：`modules/store/storage_node/chunk_index.h`、`modules/store/storage_node/chunk_index.cpp`、`CMakeLists.txt`；验收：支持 insert/update/find/list/remove，状态区分 live/staging/deleting/deleted/quarantined/corrupted/missing；依赖：T007
...
- [ ] T018 实现 per-chunk lock 和 lock striping；允许修改：`modules/store/storage_node/chunk_index.h`、`modules/store/storage_node/chunk_index.cpp`、`tests/storage_node_chunk_index_test.cpp`；验收：并发同一 chunk write/delete 串行，不同 chunk 可并行；依赖：T016、T017
- [ ] T019 定义 bounded executor / bounded IO queue；允许修改：`modules/store/storage_node/storage_executor.h`、`modules/store/storage_node/storage_executor.cpp`、`CMakeLists.txt`；验收：队列容量、timeout、cancellation、overloaded result 可测；依赖：T007
...
- [ ] T021 定义 `LocalDiskChunkStore` 构造配置和目录初始化；允许修改：`modules/store/storage_node/local_disk_chunk_store.h`、`modules/store/storage_node/local_disk_chunk_store.cpp`、`CMakeLists.txt`；验收：无效 data_dir、权限错误、目录创建失败均返回明确错误；依赖：T010、T011、T016
...
- [ ] T023 实现 `LocalDiskChunkStore::WriteChunk` 的 staging -> checksum -> flush -> publish -> index 更新路径；允许修改：`modules/store/storage_node/local_disk_chunk_store.cpp`、`tests/local_disk_chunk_store_test.cpp`；验收：checksum on write、same chunk same content idempotent、mismatch conflict、publish 前不可读；依赖：T013、T014、T015、T018、T021
- [ ] T024 实现 `LocalDiskChunkStore::ReadChunk` 和 checksum on read；允许修改：`modules/store/storage_node/local_disk_chunk_store.cpp`、`tests/local_disk_chunk_store_test.cpp`；验收：只读 live chunk，corrupted/quarantined/staging/deleting 不返回成功，checksum mismatch 标记 corrupted；依赖：T023
- [ ] T025 实现 `DeleteChunk`、`StatChunk`、`ListChunks` 的本地语义；允许修改：`modules/store/storage_node/local_disk_chunk_store.cpp`、`tests/local_disk_chunk_store_test.cpp`；验收：重复 delete 幂等，Stat/List 区分 live/staging/deleting/deleted/quarantined/corrupted/missing；依赖：T024
...
- [ ] T031 [US1] 实现 `StorageNodeService::WriteChunk` 适配层；允许修改：`modules/store/storage_node/storage_node_service.h`、`modules/store/storage_node/storage_node_service.cpp`、`CMakeLists.txt`；验收：service 只调用 `ChunkStore`，不调用 `RaftNode::ProposeMetadata()`，不决定 object commit；依赖：T030、T023
- [ ] T032 [US1] 实现 `StorageNodeClient::WriteChunk` deadline/retry/cancellation/error mapping；允许修改：`modules/store/storage_node/storage_node_client.h`、`modules/store/storage_node/storage_node_client.cpp`、`CMakeLists.txt`；验收：retry 只用于 retryable 错误，checksum mismatch/conflict 不重试；依赖：T031
- [ ] T033 [P] [US1] 实现默认 `ReplicaPolicy` 的 3 副本/最小 2 成功规则；允许修改：`modules/store/storage_node/replica_policy.h`、`modules/store/storage_node/replica_policy.cpp`、`tests/storage_placement_test.cpp`、`CMakeLists.txt`；验收：不引入 erasure coding，策略可配置但默认值固定为 3/2；依赖：T007
- [ ] T034 [US1] 实现 MVP `PlacementManager` 的静态候选节点选择；允许修改：`modules/store/storage_node/placement_manager.h`、`modules/store/storage_node/placement_manager.cpp`、`tests/storage_placement_test.cpp`；验收：能选择 `replica_count` 个节点，跳过显式 excluded nodes，输出 decision reasons；依赖：T033
...
- [ ] T043 [US2] 实现 `StorageNodeService::ReadChunk`；允许修改：`modules/store/storage_node/storage_node_service.cpp`、`modules/store/storage_node/storage_node_service.h`；验收：只从 `ChunkStore` 读取 live chunk，checksum mismatch 返回明确错误并标记 corrupted；依赖：T024、T042
- [ ] T044 [US2] 实现 `StorageNodeClient::ReadChunk` fallback 输入/输出结构；允许修改：`modules/store/storage_node/storage_node_client.cpp`、`modules/store/storage_node/storage_node_client.h`；验收：timeout/node unavailable 可尝试下一副本，checksum mismatch 不使用该副本数据；依赖：T043
- [ ] T045 [US2] 实现 read replica selection 的基础排序逻辑；允许修改：`modules/store/storage_node/replica_policy.cpp`、`modules/store/storage_node/replica_policy.h`、`tests/storage_placement_test.cpp`；验收：跳过 corrupted/unavailable/stale/overloaded candidates；依赖：T033、T044
...
- [ ] T052 [US3] 实现 `StorageNodeService::DeleteChunk`、`StatChunk`、`ListChunks` 和 `BatchDeleteChunks`；允许修改：`modules/store/storage_node/storage_node_service.cpp`、`modules/store/storage_node/storage_node_service.h`；验收：删除幂等，List 分页有界，BatchDelete 内部并发有上限；依赖：T025、T051
- [ ] T053 [US3] 实现 `StorageNodeClient` 删除和批量删除调用；允许修改：`modules/store/storage_node/storage_node_client.cpp`、`modules/store/storage_node/storage_node_client.h`；验收：partial batch result 可重试，非 retryable 错误不盲重试；依赖：T052
- [ ] T054 [US3] 实现 `GarbageCollector` 任务模型和 bounded 后台队列；允许修改：`modules/store/storage_node/garbage_collector.h`、`modules/store/storage_node/garbage_collector.cpp`、`CMakeLists.txt`；验收：GC task 有 reason、metadata_boundary、attempts、last_error、state；依赖：T019、T053
- [ ] T055 [US3] 实现 metadata-driven GC safety check；允许修改：`modules/store/storage_node/garbage_collector.cpp`、`tests/storage_delete_gc_test.cpp`；验收：任何 committed live manifest 引用的 chunk 不会被 GC 删除；依赖：T054
- [ ] T056 [US3] 覆盖 pending timeout、failed upload cleanup、AbortObject cleanup 的 GC candidate 生成；允许修改：`modules/store/storage_node/garbage_collector.cpp`、`tests/storage_delete_gc_test.cpp`；验收：未 committed 的 durable orphan 可清理，已 committed live chunk 受保护；依赖：T055
...
- [ ] T062 [US4] 定义 `StorageNodeHeartbeat`、`StorageNodeRegistry` 和 liveness 状态；允许修改：`modules/store/storage_node/storage_node_registry.h`、`modules/store/storage_node/storage_node_registry.cpp`、`CMakeLists.txt`；验收：字段包含 node_id、capacity、used、available、chunk_count、health、disk_pressure、io_error_count、load、last_seen、node_liveness；依赖：T061
- [ ] T063 [US4] 实现 `StorageNodeService` heartbeat/report/register 入口；允许修改：`modules/store/storage_node/storage_node_service.cpp`、`modules/store/storage_node/storage_node_service.h`；验收：同 sequence 幂等，stale heartbeat 不覆盖新 facts；依赖：T062
- [ ] T064 [US4] 实现 `StorageNodeClient` heartbeat/report/register 调用；允许修改：`modules/store/storage_node/storage_node_client.cpp`、`modules/store/storage_node/storage_node_client.h`；验收：control-plane unavailable/timeout 可重试，duplicate sequence 安全；依赖：T063
- [ ] T065 [US4] 将 registry facts 接入 `PlacementManager` eligibility；允许修改：`modules/store/storage_node/placement_manager.cpp`、`modules/store/storage_node/placement_manager.h`；验收：Placement 使用 capacity、health、load、disk pressure、failure-domain placeholder、hotspot signal；依赖：T060、T062
- [ ] T066 [US4] 将 registry facts 接入 read replica selection；允许修改：`modules/store/storage_node/replica_policy.cpp`、`modules/store/storage_node/replica_policy.h`；验收：read selection 降权 stale/unhealthy/overloaded replica；依赖：T045、T062
...
- [ ] T070 [US5] 实现 `LocalDiskChunkStore::RebuildIndexFromDisk`；允许修改：`modules/store/storage_node/local_disk_chunk_store.h`、`modules/store/storage_node/local_disk_chunk_store.cpp`；验收：扫描 final/staging/quarantine/deleting 并重建 ChunkIndex；依赖：T025
- [ ] T071 [US5] 实现 stale staging cleanup 和 partial write detection；允许修改：`modules/store/storage_node/local_disk_chunk_store.cpp`、`tests/storage_node_recovery_test.cpp`；验收：incomplete staging 删除或隔离，size/checksum 不一致不进入 live index；依赖：T070
- [ ] T072 [US5] 实现 corrupted chunk quarantine；允许修改：`modules/store/storage_node/local_disk_chunk_store.cpp`、`tests/storage_node_recovery_test.cpp`；验收：扫描或读取 checksum mismatch 后进入 quarantine/corrupted，不作为健康副本返回；依赖：T071
...
- [ ] T075 [US5] 覆盖 Windows long path、UTF-8 path、permission denied、disk full 错误分类；允许修改：`tests/storage_cross_platform_durability_test.cpp`、`modules/store/storage_node/durable_file.cpp`；验收：Windows 和 Linux 分支错误分类一致，路径错误不静默成功；依赖：T014、T015
...
- [ ] T082 [US6] 实现 `StorageNodeService::ScrubChunk` 和 `StorageNodeClient::ScrubChunk`；允许修改：`modules/store/storage_node/storage_node_service.cpp`、`modules/store/storage_node/storage_node_client.cpp`；验收：full checksum validation，mismatch 标记 corrupted/quarantine；依赖：T072、T081
- [ ] T083 [US6] 实现 `ScrubManager` bounded background queue；允许修改：`modules/store/storage_node/scrub_manager.h`、`modules/store/storage_node/scrub_manager.cpp`、`CMakeLists.txt`；验收：低优先级 IO，不饿死前台读写，输出 corrupted/lost/under-replicated facts；依赖：T082
- [ ] T084 [US6] 实现 `RepairManager` task model；允许修改：`modules/store/storage_node/repair_manager.h`、`modules/store/storage_node/repair_manager.cpp`、`CMakeLists.txt`；验收：task 记录 source_node、target_node、expected_checksum/size、state、progress、attempts、last_error；依赖：T083、T065
- [ ] T085 [US6] 实现 `RepairChunk` copy flow；允许修改：`modules/store/storage_node/repair_manager.cpp`、`modules/store/storage_node/storage_node_client.cpp`、`tests/storage_scrub_repair_test.cpp`；验收：source checksum verified，target durable before metadata/replica health update，corrupted source 不可用；依赖：T084
- [ ] T086 [US6] 实现 under-replicated detection；允许修改：`modules/store/storage_node/repair_manager.cpp`、`modules/store/storage_node/replica_policy.cpp`、`tests/storage_scrub_repair_test.cpp`；验收：healthy replicas 低于 policy target 时生成 repair task，低于读安全阈值返回明确风险；依赖：T085
- [ ] T087 [US6] 实现 `RebalanceManager` task model 但不接入自动后台调度；允许修改：`modules/store/storage_node/rebalance_manager.h`、`modules/store/storage_node/rebalance_manager.cpp`、`CMakeLists.txt`；验收：capacity/hotspot/new node join task 可创建，默认不自动迁移 MVP 数据；依赖：T086
- [ ] T088 [US6] 实现 rebalance copy/verify/manifest coordination 占位流程；允许修改：`modules/store/storage_node/rebalance_manager.cpp`、`tests/storage_rebalance_test.cpp`；验收：target durable before manifest update，source cleanup after metadata no longer requires source，避免 half-migrated manifest；依赖：T087
