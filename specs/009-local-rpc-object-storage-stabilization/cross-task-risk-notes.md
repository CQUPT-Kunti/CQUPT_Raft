...
## R25: Metadata Broad Regression Still Contains Unrelated Timing-Sensitive Failures That Can Block T111 Closure

Risk: T111 adds focused MetadataNode dynamic join identity/restart/registry validation in `test_node_identity` and `test_metadata_client_scenario`, but the recommended broad regression command `ctest --preset debug-tests -R "Metadata|NodeIdentity" --output-on-failure` can still fail in unrelated metadata failover/concurrency/recovery tests. During this task, `MetadataFailoverTest.ConcurrentDuplicateCreateObjectRequestsShareSameLogIndex`, `MetadataConcurrencyStressTest.AdmissionRejectsWhenInflightLimitIsReached`, and `MetadataRecoveryStressTest.RestartRecoveryAfterConcurrentWritesKeepsCommittedAndDeletedMetadataStable` produced timing/leadership-related failures outside the modified files. This blocks a clean task checkmark even when the new T111 coverage itself passes.

Mitigation: Keep T111 scoped to the added dynamic join coverage, but do not mark it complete until the broad `Metadata|NodeIdentity` regression is stable again. Follow-up work should either stabilize those timing-sensitive metadata tests or formally narrow the required regression set for join-only validation, and Phase 12 closure tasks should treat these failures as open residual risk instead of silent noise.
