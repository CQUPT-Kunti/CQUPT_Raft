...
|------|----------------|---------------------|-------|
| Local RPC example | `examples/object-storage-local-3meta-6store/qidong.sh`, `examples/object-storage-local-3meta-6store/rpc_demo.sh status`, `examples/object-storage-local-3meta-6store/rpc_demo.sh roundtrip`, `examples/object-storage-local-3meta-6store/tingzhi.sh` | Preserve 008 static 1 ViewNode + 3 MetadataNode + 6 StorageNode real roundtrip as the 009 local RPC compatibility baseline | Client remains `storage_client`; test file directory remains `tests/test_file`; report confirms `CreateWritePlan -> WriteChunk -> CommitObject -> Download -> cmp` |
